...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".agent", "skills", "pr_review")))
import pr_skill
...
class TestPRReviewSkill(unittest.TestCase):
    ...
    def test_offline_mode_success(self, mock_github, mock_auth, mock_run):
        ...
        # Setup mocks
        mock_run.return_value = MagicMock(returncode=0, stdout="Mock feedback", stderr="")

        mgr = ReviewManager(local=False, offline=True, verbose=False)
        result = mgr.trigger_review(pr_number=None, wait_seconds=0, model="gemini-2.5-pro")

        self.assertIsNotNone(result)
        self.assertEqual(result.get("status"), "success")
        self.assertIn("Offline review completed locally", result.get("message"))
        self.assertEqual(result["initial_status"]["new_item_count"], 1)
        self.assertEqual(result["initial_status"]["items"][0]["body"], "Mock feedback")
    ...
    def test_local_mode_success(self, mock_sleep, mock_github, mock_auth, mock_run, mock_detect_repo):
        ...
        mgr.check_status = MagicMock(return_value={"status": "success", "items": [], "next_step": "Fix"})

        result = mgr.trigger_review(pr_number=123, wait_seconds=0, model="gemini-2.5-pro")
    ...
    @patch("pr_skill.Auth.Token")
    @patch("pr_skill.Github")
    def test_online_mode_trigger(self, mock_github, mock_auth, mock_run, mock_detect_repo):
        ...
        def run_side_effect(*args, **kwargs):
            mock_res = MagicMock(returncode=0, stdout="0 0\n" if "rev-list" in args[0] else "")
            return mock_res
        ...
        mgr.g = mock_github.return_value
        mgr.repo = mock_repo
        mgr._poll_for_main_reviewer = MagicMock(return_value={"status": "success"})
